"""
    ...
    Description:    Create a program that iterates through a list of values. If the object is immutable, print the type
                    ...
                    the object is mutable and a number, take 10 (int) to 20 (float) percent off, print the new value,
                    and overwrite the value in the existing position. If an object is not a string, number, or tuple,
    ...
    if type(content[i]) is tuple:
        ...
        for j in range(0, len(updated_list)):
            if type(updated_list[j]) is int:
                updated_list[j] = updated_list[j] - updated_list[j] * .1
            elif type(updated_list[j]) is float:
                updated_list[j] = updated_list[j] - updated_list[j] * .2
